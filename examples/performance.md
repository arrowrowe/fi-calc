...
var J = function (job) {
  ...
  for (var index in packages) {
    ...
    var t = Date.now(); console.time(key); for (var i = 0; i < 20; i++) { fn(key, fiCalc); } console.timeEnd(key); t = Date.now() - t;
    M('  ' + key + ': ' + t.toString() + 'ms');
  }
...
setTimeout(function () { for (var i = 0; i < jobs.length; i++) { J(jobs[i]); } }, 200);
````
